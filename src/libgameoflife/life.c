...
void generational_change(char *original,int cell,char *future,int i, int j,int size )
{
...
void copy_world(char *original , char *future, settings *options)
{
    for (int i = 0; i <options->width; i++) {
        for (int j = 0; j < options->height; j++) {
            *(original+i*options->width+j) = *(future+i*options->width+j);
        }
    }
}
...
int cmp_world(char *original, char *future, settings *options )
{
    for (int i = 0; i < options->width; i++) {
        for (int j = 0; j < options->height; j++) {
            if (*(original+i*options->width+j) != *(future+i*options->width+j))
                return -1;
        }
    }
    return 0;
